...
EconomyDriver::EconomyDriver(ifstream& fin) : Driver(fin) {
    fin >> isElectric;
    fin.ignore();
}

void EconomyDriver::print() {
    cout << "Vehicle class: Economy\n";
    Driver::print();
    cout << "Is Electric: " << (isElectric ? "Yes" : "No") << "\n";
}

void EconomyDriver::save(ofstream& fout) {
    fout << "economy\n";
    Driver::save(fout);
    fout << "\n" << isElectric;
}

void EconomyDriver::edit() {
    cout << "User's current info:\n";
    print();
    cout << "Which value would you like to edit:\nn - name, c - vehicle's capacity for passengers, h - handicapped capable, t - type of vehicle, p - allow pets, w - notes, e - electric (q - cancel edit):\n";
    char value;
    cin >> value;
    cin.ignore();
    if(value == 'n') {
        ...
        cout << "Is your vehicle electric now (y/n): ";
        this->isElectric = getBool();
    } else if(value == 'q') {
        cout << "Canceled\n";
    } else { 
